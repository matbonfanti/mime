...
class sumofproduct:
    ...
    def __init__(self, xBasisLabel, yBasisLabel, xBasisFormat=None, yBasisFormat=None):
        ...
        # initialize with empty lists when the argument is not given
        if yBasisFormat is None:
            yBasisFormat = []
        if xBasisFormat is None:
            xBasisFormat = []
        ...
        for label in xBasisLabel:
            self.xBasis.append(sympy.sympify(label))
        for label in yBasisLabel:
            self.yBasis.append(sympy.sympify(label))
    ...
    def fit_data(self, Coord, V, FixZero=False):
        ...
        for point in Coord:
            if len(point) != 2:
                print(" Wrong coordinate dimension... ")
                sys.exit()

        if FixZero:
            CoordPZero = list(Coord) + [[0.0, 0.0]]
            VPZero = list(V) + [0.0]
            Weights = [1.0] * len(V) + [1000.]

            # do the fitting
            self.Coeffs, Chisquared = linearfitting(CoordPZero, VPZero, self.fullBasis, (self.x, self.y),
                                                    Weights=Weights)
    ...
    def show_1D_modelplusdata(self, Coord, Pot, yspacing=1.0):
        ...
        func = sympy.lambdify((self.x, self.y), expr, "numpy")
